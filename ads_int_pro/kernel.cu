...
void gg_main(CSRGraph& hg, CSRGraph& gg) {
	...
	if (num_tb_per_sm == 0) {
        num_tb_per_sm = 1;
        printf("!!! WARNING: Auto-tuning failed. Forcing num_tb_per_sm = 1 !!!\n");
    }
...
#define RUN_LOOP 8
	...
	for (int loop = 0; loop < RUN_LOOP; loop++) {
		...
		cudaDeviceSynchronize();
		cudaError_t err_init = cudaGetLastError();
		if (err_init != cudaSuccess) {
			printf("INIT KERNEL FAILED: %s\n", cudaGetErrorString(err_init));
			exit(1);
		}
		...
		float ave_wt;
		{
			...
			cudaDeviceSynchronize();
			cudaError_t err_prof = cudaGetLastError();
			if (err_prof != cudaSuccess) {
				printf("PROFILER KERNEL FAILED: %s\n", cudaGetErrorString(err_prof));
				exit(1);
			}
        ...
        if (err_launch != cudaSuccess) {
            ...
            printf("Error Code: %d, Reason: %s\n", err_launch, cudaGetErrorString(err_launch));
            exit(1);
        }

        cudaDeviceSynchronize(); // 强迫 CPU 等待 GPU
        
        cudaError_t err_exec = cudaGetLastError();
        if (err_exec != cudaSuccess) {
            printf("\n!!!!!! KERNEL EXECUTION FAILED !!!!!!\n");
            printf("Error Code: %d, Reason: %s\n", err_exec, cudaGetErrorString(err_exec));
            exit(1);
        }
...
int main(int argc, char *argv[]) {
	...
	// Apply Property-driven Reordering (PRO)
	// This reorders vertices by degree (descending) and edges by weight (ascending)
	// It also updates start_node to match the new vertex ID
	g.apply_property_driven_reordering(start_node, VERTEEX_REORDERING);
