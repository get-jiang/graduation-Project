...
int main( int argc, char ** argv ) {
	...
	Additional arguments:\n\
		...
		-Minimum edge weight (default: 10). E.g., -minWeight 1\n\
		-Maximum edge weight (default: 1000). E.g., -maxWeight 100";
	...
	try{
		...
		for( int iii = 1; iii < argc; ++iii ) {
			if( !strcmp(argv[iii], "-nEdges") && iii != argc-1 /*is not the last one*/)
				...
				directedGraph = false;
			else if( !strcmp(argv[iii], "-minWeight") && iii != argc-1 /*is not the last one*/)
				g_minWeight = std::stoi( std::string(argv[iii+1]) );
			else if( !strcmp(argv[iii], "-maxWeight") && iii != argc-1 /*is not the last one*/)
				g_maxWeight = std::stoi( std::string(argv[iii+1]) );
		}
		...
		std::cout << "Requested graph will have " << nEdges << " edges and " << nVertices << " vertices." << "\n" <<
				...
				"Edge weights will be random in range [" << g_minWeight << ", " << g_maxWeight << "]." << "\n";
	...
	outf << "%%MatrixMarket matrix coordinate integer general\n";
	outf << nVertices << " " << nVertices << " " << nEdges << "\n";
